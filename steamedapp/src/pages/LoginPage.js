...
import { loginUser } from '../firebase/services/auth';
...
const LoginPage = () => {
    ...
    // Check for error in URL parameters on mount and when location changes
    useEffect(() => {
        const params = new URLSearchParams(location.search);
        const errorParam = params.get('error');

        if (errorParam === 'account-disabled') {
            setError('Your account has been disabled by your teacher. Please contact them for assistance.');
    ...
    }, [location, authError, setAuthError, navigate]);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            // Pass navigate function to handle disabled account redirection
            const result = await loginUser(email, password, navigate);

            // If the function returns null, it means disabled account was handled directly
            if (result === null) {
                setLoading(false);
                return;
            }

            // For successful login, AuthContext will handle the redirects
